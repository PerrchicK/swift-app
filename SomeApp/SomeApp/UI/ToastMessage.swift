...
class ToastMessage: UIView {
    ...
    static func show(messageText: String, delay: ToastMessageLength = ToastMessageLength.short, onGone: (() -> ())? = nil) {
        ...
        let xibFileName: String = className(ToastMessage.classForCoder().self)
        let nib: UINib = UINib(nibName: xibFileName, bundle: nil)
        let toastMessage: ToastMessage = nib.instantiate(withOwner: nil, options: nil).first as! ToastMessage
